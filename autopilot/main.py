...
with sc:
    ...
    while True:
        ...
        if n:
            print(f"[UPDATE] {n} simvars changed")
            ...
            ap_master = dd.simdata["AUTOPILOT MASTER"]
            ...
            # Status output
            ap_status = "ENGAGED" if ap_master == 1.0 else "STANDBY"
            print(f"[AP {ap_status}] Pitch: {current_pitch:+6.2f}° → {desired_pitch:+6.2f}° (out: {pitch_output:+5.2f}) | Bank: {current_bank:+6.2f}° → {desired_bank:+6.2f}° (out: {bank_output:+5.2f})")

            # Send commands only when AP is engaged
            if ap_master == 1.0:
